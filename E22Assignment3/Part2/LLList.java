...
public class LLList implements List {
     ...
     * This method runs in O(n) time with O(1) additional memory.
     *
     */

    public boolean removeAll(Object item)
    {
        // Create a traversal Node pointer and a trailing Node pointer
        // Create a boolean that is equal to false and will be changed to
        // true if the parameter item equals the head Node's item.
        Node trav = head;
        Node trail = head;
        boolean remove = false;

        // Check for empty list
        if((head.next.item==null) || (length <= 0))
        {
            throw new IllegalArgumentException("The list cannot be empty!");
        }

        // Indefinate loop while the traversal Node is not null
        while(trav != null)
        {
            // If parameter item equals traversal item remove the item from the LLList
            // Set boolean remove to true
            ...
            // Update trailing Node to traversal Node and traversal Node to the next Node
            trail = trav;
            trav = trav.next;
        }

        // Return the boolean value which will be true if a Node was removed or False if not
        return remove;
    }
