...
public class Puzzle {
    ...
    {
        // Base case returns true if n equals 81 cells
        if(n>=DIM*DIM)
        {
            return true;
        }

        // Return recursive call if values are fixed
        if(valIsFixed[n/DIM][n%DIM])
        {
            return solveRB(n+1);
        }

        // Iterate possible values and place each value if it meets the conditions of Sudoku
        // If the recursive call is true return true, else remove the value and return false
        for(int i=1; i<=DIM; i++)
        { 
            if((checkRow(i, n/DIM) && (checkCol(i, n%DIM))))
            {
                if(!this.subgridHasValue[(n/DIM)/SUBGRID_DIM][(n%DIM)/SUBGRID_DIM][i])
                { 
                    //System.out.println("placed  " + i + " at " + n/DIM + " " + n%DIM);
                    placeVal(i, n/DIM, n%DIM);

                    if(solveRB(n+1)) return true;

                    //System.out.println("remove " + i + " at " + n/DIM + " " + n%DIM);
    ...
    private static void printRowSeparator() {
        ...
        // A private helper method that returns false if a value input is found
    // within the same row
