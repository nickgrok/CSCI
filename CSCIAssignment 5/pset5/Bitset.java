...
class Bitset
{
	...
	*/
    ...
    /**
     *  A method that returns the cardinality of a set to the user
     *
     *
     */ 
    public int cardinality()
    {    
        // Iterate through all values up to the maximum value
        // Create variable for byte location
        // Create variable for bit location
        // Boolean logic whether bit at byte location by bit location = 0 or not
        // If location is larger than 0 increase counter by 1
     
        int count = 0;

        for(int i=0;i<maxSize; i++)
        {
            // Same code as getBit on line 86 but applied to each iteration i
            // Each bit increases the counter by one
            int whichByte = i / 8;
            int whichBit = i % 8;
    ...
    /**
     *  A method that returns a boolean true if every element in setA is in setB
     *
     *
     */ 
    public boolean isSubset(Bitset b)
    {
        // Get the intersection of Bitset A and Bitset B
        // Test if cardinality of Bitset A equals the cardinality of the intersection of Bitset A and B
        // If they do not equal return false
        // If they do equal return true

        Bitset x = this.intersect(b);

        if(this.cardinality() != x.cardinality()) return false;
